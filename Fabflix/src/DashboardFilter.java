...
public class DashboardFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (httpRequest.getSession().getAttribute("user") == null) {
			System.out.println("Redirecting to login page b/c user attribute doesn't exist in session");
	...
	private boolean isUrlAllowedWithoutLogin(String requestURI) {
		...
		if (requestURI.endsWith("login.html") || requestURI.endsWith("login.js") 
				|| requestURI.endsWith("login") || requestURI.endsWith("loginservlet") ||
				requestURI.endsWith("_dashboard") ||requestURI.endsWith("dashboard") ||
				requestURI.endsWith("dashboard.html") ||requestURI.endsWith("dashboard.js")){
			return true;
		}
		return false;
	}
